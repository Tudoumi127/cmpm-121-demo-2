...
undoButton.addEventListener("click", () => undoRedo(lines, redoBuffer));

const redoButton = document.createElement("button");
createButtons(redoButton, "Redo", false);
redoButton.addEventListener("click", () => undoRedo(redoBuffer, lines));
...
function drawCanvasContent(ctxCan: CanvasRenderingContext2D) {
  ...
  for (const { line, stamp } of lines) {
    ...
    if (stamp) {
      ctxCan.save();
      ctxCan.translate(stamp.pos.x, stamp.pos.y);
...
canvas.addEventListener("mousemove", (pos) => {
  if (drawing) {
    ...
    cursor.pos.x = pos.offsetX;
    cursor.pos.y = pos.offsetY;
